...
class VistaUsuario(tk.Frame):
    def __init__(self, padre, controlador):
        ...
        self.pack()
        self.place(x=0, y=0, width=1100, height=650)
        self.padre.protocol("WM_DELETE_WINDOW", self.cerrar_conexion)  # Usamos un método para manejar el cierre
            ...
            if persona:
                ...
                if opcion_menu:
                    ...
                    if nombre_menu:
                        ...
                        self.db.registrar_boleta(rut, fecha_actual.strftime('%Y-%m-%d'), opcion_menu, nombre_menu)
                        print("Boleta registrada en la base de datos.")
                    else:
                        messagebox.showerror("Error", "No se encontró el nombre del menú.")
                else:
                    messagebox.showerror("Error", "No hay menú asignado para este día.")
    ...
    def cerrar_conexion(self):
            ...
            self.db.desconectar()  # Llamamos al método para cerrar la conexión
            self.padre.destroy()  # Cerramos la ventana de usuario
